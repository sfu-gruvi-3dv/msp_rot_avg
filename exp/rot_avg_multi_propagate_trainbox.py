...
class LocalGlobalVLADTrainBox(BaseTrainBox):
    ...
    def _set_network(self):
        ...
        with torch.cuda.device(self.dev_ids[0]):
            if not self.use_vae:
                ...
                self.appear_w = AppearanceFusionVAE(in_node_feat=2048, in_edge_feat=512).cuda()
    ...
    def fix_spt_overfit(self, train_dataset, epoch, num_workers=2, filtering_edge_thres=-1):
        self._prepare_eval()
        data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1, shuffle=False,
                                                  num_workers=num_workers, pin_memory=False, drop_last=True)
            ...
            with torch.cuda.device(self.dev_ids[0]):
                ...
                with torch.no_grad():
                    if self.use_vae:
                        ...
                        bi_e_node_idx, bi_e_rel_Rt = graph_util.bi_direct_edge(e_node_idx, e_rel_Rt)
                        bi_e_rel_q = rot2quaternion(bi_e_rel_Rt).to(cur_dev).detach()
                        spt_q = generate_spt_by_w(bi_e_node_idx, bi_e_rel_q, w,start_nodes[0])
                        spt_q_list = []
                        for i in range(len(list(spt_q.keys()))):
                            absq = spt_q[i].view(4)
                            spt_q_list.append(absq)
                        spt_q = torch.stack(spt_q_list)
                        init_R = quaternion2rot(spt_q.clone().cpu())
                        best_id = 0
                        choosed_start_node = start_nodes[best_id]
                        gt_q = transform_q(ori_gt_q, choosed_start_node)
                        gt_R = quaternion2rot(gt_q.clone().detach().cpu())
