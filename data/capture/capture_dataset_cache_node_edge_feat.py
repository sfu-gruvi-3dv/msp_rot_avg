...
class CaptureDataset(Dataset):
    ...
    def __init__(self, iccv_res_dir, image_dir, dataset_list, lmdb_paths=None,
        ...
        self.edge_lmdb = LMDBModel(node_edge_lmdb['edge'], lock=False, read_only=True)
        self.node_lmdb = LMDBModel(node_edge_lmdb['node'], lock=False, read_only=True)
    ...
    def __getitem__(self, idx):
        ...
        for i, imageID in enumerate(subgraph_nodes):
            ...
            try:
                node_feat = self.node_lmdb.read_ndarray_by_key('%s,%d' % (dataset_name, imageID))
            except Exception:
                print('No key found on %s,%d:%s' % (dataset_name, imageID, img_key))
        ...
        for i, edge in enumerate(subgraph_edges):
            ...
            if '%d-%d' % (n1, n2) in edge_local_feat_cache:
                ...
                edge_feat = self.edge_lmdb.read_ndarray_by_key(key='%s,%d-%d' % (dataset_name, n1, n2))
                ...
                try:
                    edge_feat = self.edge_lmdb.read_ndarray_by_key(key='%s,%d-%d' % (dataset_name, n2, n1))
                except Exception:
        ...
        return idx, img_names, torch.zeros(1), img_ori_dim, cam_Es, cam_Cs, cam_Ks, out_graph_mat, img_id2sub_id, sub_id2img_id, out_covis_mat, edge_subnode_idx, edge_type, torch.zeros(1), torch.zeros(1), edge_rel_Rt, node_feats, edge_feats
