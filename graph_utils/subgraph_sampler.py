...
class SubgraphGenerator(object):
    ...
    def addingPickedEdgeByCoverRatio(self, unpickedEdgelist, subgraphs, coverRatio = 0.1):
        pickedEdgelist = []
        for u in range(self.numOfNode):
            for v in range(u+1, self.numOfNode):
        ...
        maxnumber = 0
        for subgraph in subgraphs:
            maxnumber = max(maxnumber, len(subgraph))
        maxnumber = min( len(pickedEdgelist), int(maxnumber * coverRatio))
        while maxnumber != 0:
            maxnumber -= 1
            edge = random.choice(pickedEdgelist)
    ...
    def generationCoverEdge(self, minCoverRatio=0.1):
        ...
        while True:
            ...
            # unpickedEdgeList = self.addingPickedEdgeByCoverRatio(list(unpickedEdgeSet), subgraphs, minCoverRatio)
            unpickedEdgeList = list(unpickedEdgeSet)
            preparedEdgeList = self.mregeSplitEdgelist(unpickedEdgeList)
            gen = SubgraphGenerator.fromEdgelist(preparedEdgeList,self.maxNodeNum,self.minNodeNum,maxCoverRatio=self.maxCoverRatio,verbose=self.verbose)
            pickedNode = set()
            ...
            newPickedEdge = self.addNodeIntoSubgraphUntillMaxNode(newPickedEdge)
            cnt = 0
            for edge in newPickedEdge:
                if edge in unpickedEdgeSet:
                    unpickedEdgeSet.remove(edge)
                    cnt += 1
            print("sampler: {:d}    {:d} / {:d}, {:d}".format(len(subgraphs), cnt, len(newPickedEdge), len(unpickedEdgeSet)))
