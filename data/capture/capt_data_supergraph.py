...
class SupergraphDataset(Dataset):
    ...
    def export_subgraph_to_graph(self, edge_cache=None, path=None):
        if path is None or edge_cache is None:
            print("Cannot export the subgraphs to a None path")
            return

        edge_cache_dict = dict()
        with open(edge_cache,"rb") as fin:
            edge_cahce_dict = pickle.load(fin)
        input_file_path = os.path.split(edge_cache)[0]
        
        for idx in range(len(self)):
            dataset_name, idx, img_names, imgs, img_ori_dim, cam_Es, cam_Ks, out_graph_mat, valid_id2sub_id, sub_id2valid_id, img_id2sub_id, sub_id2img_id, out_covis_mat, edge_subnode_idx, edge_type, edge_local_matches_n1, edge_local_matches_n2, edge_rel_Rt, edge_rel_err, node_feats, edge_feats = self[idx]
            subgraph_idx = "%s_%d" % (dataset_name, idx)
            if os.path.exists(os.path.join(path,subgraph_idx)):
                shutil.rmtree(os.path.join(path,subgraph_idx))
            shutil.copytree(input_file_path, os.path.join(path, subgraph_idx))
            print("exporting %s" % (subgraph_idx))
            new_edge_cache = os.path.join(path, subgraph_idx, os.path.split(edge_cache)[-1])
            new_edge_cache_dict = dict()
            with open(new_edge_cache,"wb") as fout:
                for edge in edge_subnode_idx:
                    edge_1 = "%d-%d" % (edge[0], edge[1])
                    edge_2 = "%d-%d" % (edge[1], edge[0])
