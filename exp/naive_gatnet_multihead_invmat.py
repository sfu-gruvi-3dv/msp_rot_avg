...
from data.ambi.ambi_manual_dataaset import ManualAmbiDataset
from data.ambi.ambi_dataset import AmbiDataset, imgs2batchsamesize
from torch.utils.data import Dataset, DataLoader
from net.gat_net import MultiGATBase, EdgeClassifier
...
log_path = "./log/invmat_multihead_16_newlr/logs_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")

# Loss function: binary classification
criterion = nn.BCEWithLogitsLoss()

def accuracy(pred, gt_label, threshold=0.6):
...
train_dataset = ManualAmbiDataset(dataset_base_dir='/local-scratch7/pg/Ambi/',
...
gat = MultiGATBase(input_feat_channel=32*64*2)
...
# Optimizer
optimizer = torch.optim.Adam([{'params': gat.parameters(), 'lr': 2.0e-4},
                              {'params': vlad.parameters(), 'lr': 2.0e-4},
                              {'params': reducer.parameters(), 'lr': 2.0e-4},
                              {'params': edge_classifier.parameters(), 'lr': 2.0e-4}], lr=1.0e-4)
