...
# Network from FineNet -----------------------------------------------------------
sys.path.append('../NeuRoRA')
...
for s_itr, sample in enumerate(itr):
    ...
    # build spt
    N = len(img_names)
    spt_root = build_inlier_spt(N, e_node_idx, edge_label)
    spt_node_nums = count_tree_nodes(spt_root)
    rel_R_dict = build_rel_R_dict(e_node_idx, e_rel_Rt)
    spt_init_R = build_init_pose(N, e_node_idx, rel_R_dict)
    spt_init_q = cam_opt_gpu.rot2quaternion(spt_init_R[:, :3, :3])

    rel_Es = rel_ref_2_E_(spt_root.attr, Es)
