...
from exp.make_test_dataset import make_dataset, datasets_config_filename
...
parser.add_argument("--config_file", default="/mnt/Exp_5/yfcc_init_opt_cache/train_json_config/yfcc_trevi_fountain_1.json")
parser.add_argument("--dev",default=1, type=int)

args = parser.parse_args()
print("input json: {:s}".format(args.config_file))


json_file_path = './train_config/yfcc_80nodes_dump_feat.json'
if args.config_file:
    json_file_path = args.config_file

delete_lmdb = True

run_dev_ids = [1, 1]
if args.dev:
    run_dev_ids = [args.dev, args.dev]
...
ckpt_dict = {
    ...
    'ckpt': '/mnt/Tango/pg/pg_akt_same_size/bce_yfcc_logs/Oct27_22-56-09_cs-guv-gpu02_yfcc_train_3/checkpoints/iter_005162.pth.tar'
}
...
valid_set = make_dataset(json_file_path, load_img=True, load_node_edge_feat=False)
...
def add_samples_to_lmdb(dataset, lmdb, processed_edge_dict, processed_node_dict):
    ...
    for sample in train_loader:
        dataset_name, idx, img_names, imgs, img_ori_dim, cam_Es, cam_Ks, _, valid_id2sub_id, sub_id2valid_id, img_id2sub_id, sub_id2img_id, _, edge_subnode_idx, edge_type, edge_local_matches_n1, edge_local_matches_n2, edge_rel_Rt,_,  _, _ = sample
        ...
        if len(process_edge_idx) > 0:
            ...
            with torch.no_grad():
                node_feats = prior_box.cache_vlad_feats(imgs, max_imgs_per_batch=max_batch_size)
