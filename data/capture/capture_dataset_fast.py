...
class CaptureDataset(Dataset):
    ...
    def __init__(self, iccv_res_dir, image_dir, dataset_list, lmdb_paths=None,
        ...
        for ds in dataset_list:
            ...
            frame_list = read_image_list(os.path.join(iccv_res_dir, dataset_name, bundle_prefix + '.list.txt'))
        ...
        for ds in tqdm(dataset_list):
            ...
            Es, Cs = read_poses(os.path.join(iccv_res_dir, dataset_name, bundle_prefix))
            self.Es[dataset_name] = Es
            self.Cs[dataset_name] = Cs

            inoutMat = np.load(os.path.join(iccv_res_dir, dataset_name, 'inoutMat.npy'))
            covis_map = np.load(os.path.join(iccv_res_dir, dataset_name, 'covis.npy'))
        ...
        if sample_res_cache is None or not os.path.exists(sample_res_cache):
            ...
            if sample_res_cache is not None:
                ...
                print('[Captured Init] Save subgraph fast cache to %s.' % sample_res_cache)

        elif os.path.exists(sample_res_cache):
            with open(sample_res_cache, 'rb') as f:
                s = pickle.load(f)
                self.samples, self.edge_sampler = s
            print('[Captured Init] Load subgraph fast cache from %s.' % sample_res_cache)
...
if __name__ == '__main__':
    ...
    dataset = CaptureDataset(iccv_res_dir=iccv_res_dir, image_dir=image_dir, dataset_list=datalist)

    imgs, img_ori_dim, cam_Es, cam_Cs, cam_Ks, out_graph_mat, img_id2sub_id, sub_id2img_id, out_covis_mat, edge_subnode_idx, edge_type, edge_local_matches_n1, edge_local_matches_n2 = \
    dataset[0]
    print(edge_type)
