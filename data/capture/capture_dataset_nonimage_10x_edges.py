...
data_meta_dict = {
    ...
    '1dsfm': {
        'calib': 'raw_data/calibration.txt',
        'frame_list': 'raw_data/ImageList.txt',
        'recon': 'raw_data/gt_bundle.out',
    }
}


class CaptureDataset_10x_edges(Dataset):
    ...
    def __init__(self, dataset_dir, dataset_list, img_lmdb_paths=None,
        ...
        if self.load_node_edge_feat is True:
            if node_edge_lmdb_path is None:
                raise Exception(
                    'Can not found node edge feature lmdb: %s' % node_edge_lmdb_path)
            self.node_feat_lmdb = LMDBModel(node_edge_lmdb_path)
            node_edge_meta_path = node_edge_lmdb_path.split('.lmdb')[
                0] + '.bin'
        ...
        for ds in dataset_list:
            ...
            frame_list = read_image_list(os.path.join(
                dataset_dir, dataset_name, data_meta_dict[bundle_prefix]['frame_list']))
            self.frame_list[dataset_name] = frame_list

            # camera calibration
            K, img_dim = read_calibration(os.path.join(
                dataset_dir, dataset_name, data_meta_dict[bundle_prefix]['calib']))
        ...
        for ds in tqdm(dataset_list):
            ...
            # read camera poses
            Es, _ = read_poses(os.path.join(
                dataset_dir, dataset_name, data_meta_dict[bundle_prefix]['recon']))
            self.Es[dataset_name] = Es

            # graph connectivity and inlier/outlier marker
            inoutMat = np.load(os.path.join(
                dataset_dir, dataset_name, 'inoutMat.npy'))
            covis_map = np.load(os.path.join(
                dataset_dir, dataset_name, 'covis.npy'))
        ...
        """
        self.edge_sampler = {}
        # (dataset_id, sub-graph sample_id)
        self.samples = []
        ...
        if sample_res_cache is None or not os.path.exists(sample_res_cache):
            ...
            for ds_id, ds in enumerate(dataset_list):
                ...
                all_edge_sets = dict()
                # edge_feat_pos_cache = self.edge_local_feat_cache[dataset_name]
                for edge in edge_feat_pos_cache:
                    edge = [int(x) for x in edge.split("-")]
                    inoutMat[edge[0], edge[1]] = inoutMat[edge[1], edge[0]] = 1
                    all_edge_sets[(min(edge[0], edge[1]),
                                   max(edge[0], edge[1]))] = 1
                ...
                gen.appendUntilnxEdges(
                    all_edge_sets, use_undefine=self.sampling_undefined_edge, get_max_node=False)
                print("test inoutmat")
    ...
    def __getitem__(self, idx):
        ...
        for i, imageID in enumerate(subgraph_nodes):
            ...
            if self.load_img is True:
                if self.use_lmdb is True:
                    ...
                    img = self.lmdb_db.read_ndarray_by_key(
                        img_key, dtype=np.uint8)
            ...
            if self.load_node_edge_feat:
                ...
                node_feat = self.node_feat_lmdb.read_ndarray_by_key(
                    key).ravel()
        ...
        for i, edge in enumerate(subgraph_edges):
            ...
            if '%d-%d' % (n1, n2) in edge_local_feat_cache:
                ...
                raise Exception("edge not found %s (%d, %d)" %
                                (dataset_name, n1, n2))
            ...
            if self.load_node_edge_feat:
                ...
                if key1 in self.edge_feat_dict:
                    ...
                    edge_feats.append(torch.from_numpy(
                        edge_feat.ravel()).unsqueeze(0))
