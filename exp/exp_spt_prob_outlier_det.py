...
for s_itr, sample in enumerate(itr):
    ...
    with torch.no_grad():
        ...
        for e_i, e in enumerate(e_node_idx):
            ...
            e_rel_R = e_rel_Rt[e_i]
            if out_edge_res_[e_i].item() > 0.6:
                fi_node_idx.append(e_node_idx[e_i])
                fi_e_rel_R.append(e_rel_R)
                fi_e_label.append(edge_label[0, e_i].item())
                fi_e_probs.append(out_edge_res_[e_i].item())
        fi_e_label = torch.Tensor(fi_e_label).int().unsqueeze(0)
        fi_e_probs = torch.Tensor(fi_e_probs).view(-1)

        new_sample = idx, img_names, img_lists, img_dims, Es, Cs, Ks, out_graph, sub2id, id2sub, covis_mat, fi_node_idx, edge_label, e_keypt_n1, e_keypt_n2, fi_e_rel_R

    spt_init_q, pred_q, gt_q, loss, spt_root, spt_node_nums, pred_edge_prob = box.forward_once(new_sample[2:],
                                                                                               build_inlier=False,
                                                                                               edge_prob=fi_e_probs)

    print('I:', s_itr, 'Idx:', idx, ' Outliers:', (edge_label == 0).sum(), 'Inliers:', (edge_label == 1).sum(),
          'SPT_inlier_nodes:', spt_node_nums)
    ...
    edge_errs = dict()
    init_inlier_edges = []
    for ei, (n1, n2) in enumerate(fi_node_idx):
        label = fi_e_label[0][ei].item()
        n1 = fi_node_idx[ei][0].item()
        n2 = fi_node_idx[ei][1].item()
        rel_R_obs = fi_e_rel_R[ei]
    ...
    for ix, img in enumerate(img_lists):
        img = img[0]
        delta_angle = delta[ix]
        name = img_names[ix][0].split('/')[-1].split('.jpg')[0]
        G.add_node(ix, img=img, name=name, unormlize_tensor=True, text='%.2f' % delta_angle)
    ...
    for ei, (n1, n2) in enumerate(fi_node_idx):
        ...
        n1 = fi_node_idx[ei][0].item()
        n2 = fi_node_idx[ei][1].item()
        G.add_edge(n1, n2, pred_prob=prob, gt_inlier=label, pred_prob_thres=0.6)

    for (n1, n2) in init_inlier_edges:
        G.add_mark_edge(n1, n2)

    G.draw_spring_layout(draw=False, save_fig_path='/mnt/Exp_5/vis/bottles/spt_inlier_%d_finenet.pdf' % s_itr)
    G.draw_spring_layout_marker(draw=False,
                                save_fig_path='/mnt/Exp_5/vis/bottles/spt_inlier_initspt_%d_finenet.pdf' % s_itr,
                                update_pos=False)
    # break