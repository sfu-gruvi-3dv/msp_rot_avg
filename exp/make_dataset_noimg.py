...
def clean_cache():
    paths = [os.path.join(onedsfm_res_dir, 'subnode_sampling_o_train.bin'),
             ...
             os.path.join(cap_res_dir, 'subnode_sampling_c_train_64nodes.bin'),
             os.path.join(cap_res_dir, 'subnode_sampling_c_valid_64nodes.bin')
             ]
    for path in paths:
...
def make_dataset():
    """ Captured Dataset -----------------------------------------------------------------------------------------------
    """
    train_c_datalist = [{'name': 'sv_seq5', 'bundle_prefix': 'bundle'},
                        ...
                        # {'name': 'KingsCollege_jpg', 'bundle_prefix': 'bundle'},
                        # {'name': 'StMarysChurch_jpg', 'bundle_prefix': 'bundle'},
                        ]
    ...
    valid_c_datalist = [
        ...
        {'name': 'sv_statue2', 'bundle_prefix': 'bundle'},
        {'name': 'ambi_cup', 'bundle_prefix': 'bundle'},
        # {'name': 'GreatCourt_jpg', 'bundle_prefix': 'bundle'},
    ]

    train_c_dataset = CaptureDataset(dataset_dir=cap_res_dir,
                                     image_dir=cap_res_dir,
                                     # lmdb_paths=cap_res_lmdb_cache,
                                     dataset_list=train_c_datalist,
                                     img_max_dim=400,
                                     sample_res_cache=os.path.join(cap_res_dir, 'subnode_sampling_c_train_64nodes.bin'),
                                     sampling_num_range=[150, 300],
                                     #  num_nodes_mask_path=n_spt_node_mask,
                                     sub_graph_nodes=64)

    valid_c_dataset = CaptureDataset(dataset_dir=cap_res_dir,
                                     image_dir=cap_res_dir,
                                     # lmdb_paths=cap_res_lmdb_cache,
                                     dataset_list=valid_c_datalist,
                                     img_max_dim=400,
                                     sample_res_cache=os.path.join(cap_res_dir, 'subnode_sampling_c_valid_64nodes.bin'),
                                     sampling_num_range=[150, 300],
                                     sub_graph_nodes=64)
